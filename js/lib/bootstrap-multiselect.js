    ...
    Multiselect.prototype = {
        ...
        defaults: {
            ...
            onDropdownShow: function(event) {
              if (this.$select[0].id === 'fc') {
                globalFCSelectorValue = $('#fc').val();
              }
              if (this.$select[0].id === 'hub') {
                globalHubSelectorValue = $('#hub').val();
              }
              if (this.$select[0].id === 'carrier') {
                globalCarrierSelectorValue = $('#carrier').val();
              }
              if (this.$select[0].id === 'state') {
                globalStateSelectorValue = $('#state').val();
              }
              // ENABLE: City
              if (this.$select[0].id === 'city') {
                globalCitySelectorValue = $('#city').val();
              }
            },
            // Triggered immediately when dropdown hidden
            onDropdownHide: function(event) {
              if (this.$select[0].id === 'fc') {
                $('#fc').trigger('fcSelectorChanged');
              }
              if (this.$select[0].id === 'hub') {
                $('#hub').trigger('hubSelectorChanged');
              }
              if (this.$select[0].id === 'carrier') {
                $('#carrier').trigger('carrierSelectorChanged');
              }
              if (this.$select[0].id === 'state') {
                $('#state').trigger('stateSelectorChanged');
              }
              // ENABLE: City
              if (this.$select[0].id === 'city') {
                $('#city').trigger('citySelectorChanged');
              }
            },
